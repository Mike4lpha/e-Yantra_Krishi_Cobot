...
class UR5_Manipulation(Node):
    def __init__(self):
        ...
        self.delivery_pub = self.create_publisher(Bool, '/fertilizer_delivered', 10)
    ...
    def call_detach_service(self, model_name): 
        ...
        if model_name == 'fertiliser_can':  # This is your dock delivery
            delivered_msg = Bool()
            delivered_msg.data = True
            self.delivery_pub.publish(delivered_msg)
            self.get_logger().info("Fertilizer loaded onto Ebot.")
