...
class ShapeDetection(Node):
    ...
    def stop_detection_callback(self, msg):
        ...
        if not self.stop_detection:
            self.get_logger().info("Shape detection ENABLED.")
    ...
    def process_left_side(self, points, window_name):
        ...
        pts_img = pts_img[pts_img[:, 1] < 300] ### 299
        pts_img = pts_img[
    ...
    def merge_lines(self, lines, angle_thresh_rad=np.deg2rad(5), rho_thresh_px=30):
        if lines is None:
            return None

        # Convert lines to (rho, theta) form
        polar_lines = []
        ...
        for rho, theta in polar_lines:
            ...
            for i, (mrho, mtheta, count) in enumerate(merged):
                if (abs(rho - mrho) < rho_thresh_px and
                    abs(theta - mtheta) < angle_thresh_rad):

                    # Merge by averaging
                    new_rho = (mrho * count + rho) / (count + 1)
                    new_theta = (mtheta * count + theta) / (count + 1)
                    merged[i] = (new_rho, new_theta, count + 1)
                    found = True
                    break
        ...
        # Convert back to line endpoints
        result = []
        for rho, theta, _ in merged:
            a = np.cos(theta)
            b = np.sin(theta)
    ...
    def check_publish_odom(self):
        if self.detected_shape and self.shape_detected_time is not None:
            elapsed_time = (self.get_clock().now() - self.shape_detected_time).nanoseconds / 1e9
            if elapsed_time > 3.0 and not self.flag:
