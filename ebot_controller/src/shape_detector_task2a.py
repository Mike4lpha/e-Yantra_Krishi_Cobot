...
class ShapeDetection(Node):
    ...
    def odom_callback(self, msg):
        ...
        self.y_ok = -4.185772005268209 <= y <= 0.13508567138270067
        self.ignore_zone = -2.80 < y < -2.00 and 0.21 < x < 0.29
    ...
    def process_left_side(self, points, window_name):
        ...
        if shape in ["Square", "Triangle"] and not self.shape_detected_left:
            ...
            self.shape_detected_time = time.time()
    ...
    def process_right_side(self, points, window_name):
        ...
        if self.y_ok and not self.ignore_zone:
            gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
            blurred = cv2.GaussianBlur(gray, (5, 5), 0)
            edges = cv2.Canny(blurred, 50, 150)
        ...
        if shape in ["Square", "Triangle"] and not self.shape_detected_right:
            ...
            self.get_logger().info("Robot stopped due to shape detection.")
    ...
    def merge_lines(self, lines, angle_thresh_rad=np.deg2rad(10), rho_thresh_px=20):
        if lines is None:
            return None
    ...
    def check_reset_condition(self):
        if self.detected_shape and self.shape_detected_time is not None:
            elapsed_time = time.time() - self.shape_detected_time
            if elapsed_time > 7.0:
    ...
    def check_publish_odom(self):
        if self.detected_shape and self.shape_detected_time is not None:
            elapsed_time = time.time() - self.shape_detected_time
            if elapsed_time > 2.5 and not self.flag:
    ...
    def publish_odom(self):
        ...
        msg = String()
        msg.data = f"{self.status},{x:.2f},{y:.2f}"
        self.detection_pub.publish(msg)
        self.get_logger().info(f"Detection published: {msg.data}")
        self.flag = True
