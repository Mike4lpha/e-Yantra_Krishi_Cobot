...
class WaypointController(Node):
    ...
    def __init__(self):
        ...
        self.unload_sub = self.create_subscription(Bool, '/fertilizer_unloaded', self.fertilizer_unloaded_callback, 10)
    ...
    def fertilizer_unloaded_callback(self, msg):
        if msg.data:
            self.get_logger().info("fertilzer unloading.")
            self.received_item = True
            self.waiting_for_item = False
            self.post_delivery_end_time = self.get_clock().now().seconds_nanoseconds()[0] + 1.0
    ...
    # def limit_velocity(self, twist):
    #     MAX_LIN = 0.5
    #     MAX_ANG = 1.0

    #     twist.linear.x = np.clip(twist.linear.x, 0.0, MAX_LIN)
    #     twist.angular.z = np.clip(twist.angular.z, -MAX_ANG, MAX_ANG)
    #     return twist
    ...
    def control_loop(self):
        ...
        if not self.fixing_yaw:
            ...
            if abs(final_yaw_error) > 0.05:
                ...
                if self.current_waypoint_idx == 8:
                    msg = String()
                    msg.data = f"DOCK_STATION,{self.pose[0]:.2f},{self.pose[1]:.2f},0"
                    self.detection_pub.publish(msg)
                    self.get_logger().info("DOCK_STATION Reached.")
                    ...
                    self.get_logger().info("Dock reached...Unloading")
