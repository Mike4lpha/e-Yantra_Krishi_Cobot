...
class WaypointController(Node):
    ...
    def __init__(self):
        ...
        self.waypoints = [
            ...
            (0.26, -1.95, 1.57),
            (0.26, 1.35, 1.57),
            (-1.66, 1.24, 3.14),  ## Or (-1.53, 1.24, -1.57)
            (-1.48, -0.67, -1.57),
            (-1.53, -6.61, -1.57 )
        ]
    ...
    def detection_callback(self, msg: String):
        self.get_logger().info(f"Detection received: {msg.data}")
        ...
        if not self.fixing_yaw:
            if distance > 0.1:
                if abs(yaw_error) > np.deg2rad(10):
                    twist.linear.x = 0.0
                    twist.angular.z = np.clip(self.kp_angular * yaw_error, -2.0, 1.0)
                else:
            ...
            if abs(final_yaw_error) > 0.05:
                ...
                if self.current_waypoint_idx == 1:
                    msg = String()
                    msg.data = f"DOCK_STATION,{self.pose[0]:.2f},{self.pose[1]:.2f}"
